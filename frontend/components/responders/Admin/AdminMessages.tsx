// components/responders/Admin/Messages.tsx
...
type MessagesNavigationProp = NativeStackNavigationProp<RootStackParamList, 'AdminMessages'>;
...
const AdminMessages: React.FC<MessagesProps> = ({ navigation }) => {
  ...
  const fetchAdminGroupChat = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/admin-groupchat`);
  ...
  const sendMessage = async () => {
    ...
    try {
      if (replyingTo) {
        ...
        // Send a new message
        const response = await axios.post(`${API_URL}/api/send-admin-message`, {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchAdminGroupChat, 30000);
  ...
  const renderMessage = (message: Message) => (
    <View key={message._id}>
      ...
      >
        ...
        <Text style={[
          styles.messageText,
          message.sender === 'Admin' && styles.adminMessageText
        ]}>{message.text}</Text>
        <View style={styles.messageFooter}>
          <Text style={[
            styles.messageTime,
            message.sender === 'Admin' && styles.adminMessageTime
          ]}>
          ...
          >
            <Ionicons 
              name="arrow-undo" 
              size={16} 
              color={message.sender === 'Admin' ? 'white' : '#666'} 
            />
          </TouchableOpacity>
        </View>
      </View>
