...
const BarangaySidebar = ({ isOpen, setIsOpen, userEmail, onLogout }: any) => {
  ...
  const [activeRoute, setActiveRoute] = useState('BarangayOngoing');
...
// Main Component
const BarangayOngoing = () => {
  const navigation = useNavigation<any>();
  const [ongoingReports, setOngoingReports] = useState<Report[]>([]);
  ...
  const fetchOngoingReports = async () => {
    try {
      ...
      setOngoingReports(data);
    } catch (error) {
      console.error('Error fetching ongoing reports:', error);
      Alert.alert('Error', 'Failed to fetch ongoing reports');
    }
  };
      ...
      <ScrollView style={styles.main}>
        <View style={styles.content}>
          <View style={styles.pageHeader}>
            <Icon name="spinner" size={28} color="#0077b6" />
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={true}>
              <View style={styles.table}>
                ...
                <View style={styles.tableRow}>
                  ...
                  <Text style={[styles.tableHeader, styles.colDate]}>Date Ongoing</Text>
                ...
                {ongoingReports.length > 0 ? (
                  ongoingReports.map((report, index) => (
                    ...
                    >
                      ...
                      <Text style={[styles.tableCell, styles.colDate]}>{formatDate(report.dateOngoing)}</Text>
...
export default BarangayOngoing;