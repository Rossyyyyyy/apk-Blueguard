...
const BarangaySidebar = ({ isOpen, setIsOpen, userEmail, onLogout }: any) => {
  ...
  const [activeRoute, setActiveRoute] = useState('BarangayCancelled');
...
// Main Component
const BarangayCancelled = () => {
  const navigation = useNavigation<any>();
  const [cancelledReports, setCancelledReports] = useState<Report[]>([]);
  ...
  const fetchCancelledReports = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/cancelled-reports`);
      const data = await response.json();
      setCancelledReports(data);
      ...
      Alert.alert('Error', 'Failed to fetch cancelled reports');
    }
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await fetch(`${API_BASE_URL}/api/cancelled-reports/${id}`, {
                method: 'DELETE',
              });

              const result = await response.json();
              if (response.ok) {
                setCancelledReports((prevReports) =>
      ...
      <ScrollView style={styles.main}>
        <View style={styles.content}>
          <View style={styles.pageHeader}>
            <Icon name="times-circle" size={28} color="#E15759" />
            <Text style={styles.pageTitle}>Cancelled Reports</Text>
          </View>
          ...
          {/* Cancelled Reports Card */}
          <View style={styles.cancelledCard}>
            <View style={styles.cardIconContainer}>
              <Icon name="times-circle" size={40} color="#fff" />
            </View>
            <View style={styles.cardContent}>
              <Text style={styles.cardTitle}>Total Cancelled Reports</Text>
              <Text style={styles.cardValue}>{cancelledReports.length}</Text>
            </View>
          </View>
          ...
          <View style={styles.tableSection}>
            <Text style={styles.tableSectionTitle}>Cancelled Reports Details</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={true}>
              <View style={styles.table}>
                ...
                <View style={styles.tableRow}>
                  ...
                  <Text style={[styles.tableHeader, styles.colDate]}>Date Cancelled</Text>
                  <Text style={[styles.tableHeader, styles.colAction]}>Actions</Text>
                </View>

                {/* Table Body */}
                {cancelledReports.length > 0 ? (
                  cancelledReports.map((report, index) => (
                    <View 
                      key={report._id} 
                      style={[
                        styles.tableRow, 
                    ...
                    >
                      ...
                      <Text style={[styles.tableCell, styles.colDate]}>{formatDate(report.dateCancelled)}</Text>
                  ...
                  <View style={styles.noDataContainer}>
                    ...
                    <Text style={styles.noDataText}>No cancelled reports found.</Text>
                  </View>
                )}
              </View>
            </ScrollView>
...
export default BarangayCancelled;